...
- **P2（未知の車種名だけで測り直す）は実施済み。**→ `docs/2026-08-29-feature-unseen.md`。
  機能A の既定エンコーダは未知語 3,730 で文字TF-IDF 側（3,715）に着地し、
  既知語でも 2,464 と強く、**単一の表現で両取りになっていた**。
- 埋め込みと文字TF-IDF の併用（従来の最良 2,596）は、`make_lgbm` の extra フックで
  手組みすれば 2,588 まで出る（P2 の F(d)'）。ただし上積みは振れ幅と同程度なので、
  `Feature(encoder=[...])` として API に入れる優先度は高くない。
